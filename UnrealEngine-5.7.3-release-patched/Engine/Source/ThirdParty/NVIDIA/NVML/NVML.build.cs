...
public class NVML : ModuleRules
{
	...
	public NVML(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		else if (Target.Platform == UnrealTargetPlatform.Win64 && Target.Architecture != UnrealArch.Arm64)
		{
		...
		if (DllName != "" && File.Exists(Path.Combine(DllPath, DllName)))
		{
			string public_rt_lib_path =
				(Target.Configuration == UnrealTargetConfiguration.Debug && Target.bDebugBuildsActuallyUseDebugCRT)?
					Path.Combine(ProjectBinariesDir, ConfigFolder)
					: ProjectBinariesDir;
			RuntimeDependencies.Add(
					Path.Combine(public_rt_lib_path, DllName),
					Path.Combine(DllPath, DllName),
					StagedFileType.NonUFS);

			PublicRuntimeLibraryPaths.Add(public_rt_lib_path);
			PublicDelayLoadDLLs.Add(DllName);
		}
	}
}
